...
fn model_count_captures(
    ...
    let haystack = &*b.haystack;
    let mut caps = re.locations();
    timer::run(b, || {
        let mut at = 0;
        let mut count = 0;
        while let Some(m) = re.read_captures_at(&mut caps, haystack, at) {
            for i in 0..caps.len() {
                if caps.pos(i).is_some() {
                    count += 1;
                }
            }
...
fn model_grep_captures(
    ...
    timer::run(b, || {
        ...
        for line in haystack.lines() {
            let mut at = 0;
            while let Some(m) = re.read_captures_at(&mut caps, line, at) {
                for i in 0..caps.len() {
                    if caps.pos(i).is_some() {
                        count += 1;
                    }
