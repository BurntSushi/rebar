...
struct Benchmark {
	...
	void read_from_stdin() {
		...
		while (buf.length > 0) {
			auto key = split();
			switch (key) {
				case "name":  				mixin(parseArg!(string, "name")); break;
				case "model": 				mixin(parseArg!(string, "model")); break;
				case "pattern": 			{	this.regex.patterns ~= readVal();
												break; }
				case "case-insensitive": 	mixin(parseArg!(bool, "regex.case_insensitive")); break;
				case "unicode": 			mixin(parseArg!(bool, "regex.unicode")); break;
				case "haystack":			{	this.haystack = readVal();
												break; }
				case "max-iters":			mixin(parseArg!(size_t, "max_iters")); break;
				case "max-warmup-iters":	mixin(parseArg!(size_t, "max_warmup_iters")); break;
				case "max-time":			{	this.max_time = dur!"nsecs"( to!ulong( readVal() ) );
												break; }
				case "max-warmup-time":		{	this.max_warmup_time = dur!"nsecs"( to!ulong( readVal() ) );
												break; }
				default: {
					throw new Exception("unknown key: '" ~ to!string(key) ~ "'");
				}
			}
		}
	}
...
void run_and_count(T)(ref Benchmark b, size_t delegate(ref T) count, T delegate() bench) {
...
void main(string[] args) {
	...
	switch (b.model) {
		...
		default:
			throw new Exception("unrecognized benchmark model '" ~ b.model ~ "'");
	}
}
	...
	run(b,
		() => haystack.matchAll(re)
				.map!((Captures!string m) {
					// need to check here and subtract one since captures includes
					// the whole match in it's length.
