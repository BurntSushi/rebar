...
impl Regex {
    ...
    pub fn is_match(&self, haystack: &[u8], start: usize, end: usize) -> bool {
    ...
    pub fn find(
        ...
        if matched {
            Some((match_start as usize, match_end as usize))
        } else {
            None
        }
    }
    ...
    pub fn find_iter<'r, 'h>(
        ...
        FindMatches { re: self, haystack, at: 0, last_match_end: None }
    }
    ...
    pub fn captures(
        ...
        if matched {
            ...
            caps.haystack_start_addr = Some(haystack.as_ptr() as usize);
        }
        matched
    }
}
...
impl<'r, 'h> Iterator for FindMatches<'r, 'h> {
    ...
    fn next(&mut self) -> Option<(usize, usize)> {
        let mut m =
            self.re.find(self.haystack, self.at, self.haystack.len())?;
        if m.0 >= m.1 {
...
impl<'r, 'h> FindMatches<'r, 'h> {
    ...
    fn handle_overlapping_empty_match(
        ...
        if Some(m.1) == self.last_match_end {
            ...
            m = self.re.find(self.haystack, self.at, self.haystack.len())?;
        }
        Some(m)
    }
}
...
mod tests {
    ...
    fn captures() {
        ...
        assert!(re.captures(hay.as_bytes(), 0, hay.len(), &mut caps));
        assert_eq!(Some((3, 9)), caps.get_group(0));
        assert_eq!(None, caps.get_group(1));
        assert_eq!(Some((6, 9)), caps.get_group(2));
    }
}
